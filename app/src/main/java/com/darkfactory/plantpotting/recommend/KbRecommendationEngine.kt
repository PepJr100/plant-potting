...
@Singleton
class KbRecommendationEngine @Inject constructor(
    ...
    private fun single(species: Species, mapping: ArchetypeMapping.Single): Recommendation {
        val archetype = kb.archetypes[mapping.archetypeId]
    ...
    private fun blend(species: Species, mapping: ArchetypeMapping.Blend): Recommendation {
        val primary = kb.archetypes[mapping.primaryArchetypeId]
            ?: throw IllegalStateException(
                "species ${species.id}: primary archetype '${mapping.primaryArchetypeId}' missing",
            )
        val secondary = kb.archetypes[mapping.secondaryArchetypeId]
            ?: throw IllegalStateException(
                "species ${species.id}: secondary archetype '${mapping.secondaryArchetypeId}' missing",
            )
        val recipe = blendRecipes(
    ...
    private fun buildSingleRationale(species: Species, archetype: Archetype): String {
    ...
    private fun normaliseToIntPercentages(
        ...
        val biggest = accum.entries.maxByOrNull { it.value }?.key
