...
- [x] **G5 — Constraints preserved.** `verifyNoNetworking` and `scripts/check-stub-isolation.sh` stay GREEN; no KB edits; no model/bundle/`ACTIVE_MODEL_ROOT` change; no committed image data; the `PlantIdentifier`/`IdentificationResult` seam stays frozen. → all verified GREEN; seam, KB, model, and `ACTIVE_MODEL_ROOT` untouched (only `CameraScreen.kt` + camera tests changed).

## Non-goals / scope boundaries

- [x] **No ML training or fine-tuning in this sprint.** This spike only decides *if* a later fine-tuning sprint is viable. No `.h5`/checkpoint training, no transfer-learning runs, no head surgery. → upheld.
- [x] **No model work.** No model conversion, no new bundle under `app/src/main/assets/ml/`, no change to the `ACTIVE_MODEL_ROOT` default (`ml/house_plant_species_mobilenetv2`), no manifest/label-map/threshold edits. → upheld.
- [x] **No KB edits.** `app/src/main/assets/kb/species.json` and `archetypes.json` are LOCKED (read-only input to the report). → upheld (read-only).
- [x] **No bulk image download; no images committed.** Assess counts via APIs/UI/search only. NO sample images in the repo or APK. Any tiny manual eyeball check stays **off-repo** (local scratch dir outside the project tree, never staged). → upheld (count probes only; no eyeball download this pass).
- [x] **No new fixtures.** The 8 existing `identify-fixtures/` JPGs are untouched; the spike only references them to define the disjoint-split exclusion set. → upheld.
- [x] **Seam frozen.** `PlantIdentifier.identify(jpeg): IdentificationResult` and `IdentificationResult` unchanged. The UX fix is confined to the camera screen / `CameraViewModel` state. → upheld (only `CameraScreen.kt` lifecycle observer + tests).
- [x] **No production networking.** Any API-query tooling lives under `docs/sprints/evidence/PLANTPOTTING-0008/` or `scripts/`, is **never** invoked from a Gradle app task, and never runs on device. → upheld; `verifyNoNetworking` GREEN.
- [x] **No version bumps** (AGP/Kotlin/Compose/Hilt/TFLite). → upheld.
- [x] **Do not broaden the idea-inbox UX work** beyond the shutter re-enable bug. → upheld (single, minimal fix).
...
- [x] Write `docs/sprints/results/PLANTPOTTING-0008.md`: the verdict, per-species count summary, the chosen hand-off, the UX-fix status, and all gate results, plus an explicit statement that **no training data or model assets were committed**. Flip the ledger only after every gate is green. → written; ledger flipped to `done` after all gates GREEN.
...
- [x] `ktlintCheck` GREEN (the only code change is the camera screen/VM + its tests).
- [x] `testDebugUnitTest` GREEN — including the new `CameraViewModelTest` case (`resetFromTerminalSuccessReturnsToIdle`).
- [x] `verifyNoNetworking` GREEN — confirm no new network surface; any API-query script is outside the app source set and not wired to a Gradle app task. → GREEN; this spike used no committed script (live probes ran via the orchestrator's web tools, off-build-pipeline).
- [x] `scripts/check-stub-isolation.sh` GREEN — the UX fix introduces no fake/test identifier into app wiring. → "stub isolation OK".
- [x] Instrumented camera tests on `pixel6Api34` GMD: the new shutter-on-return test + the existing bind/bound/smoke tests, all green. → **6/6 GREEN** (`CameraShutterOnReturnTest`, `CameraScreenBindStateTest`, `CameraScreenBoundStateTest`, `CameraScreenSmokeTest`×2, `CameraPreviewLayoutTest`).
- [x] `scripts/integration-flow.ps1` cold / warm / buildonly — confirm expected artifacts unchanged (no model/KB/manifest change ⇒ no re-baseline expected). → **buildonly GREEN** + **device-aware (warm) GREEN** (manifest diff passed; model asset present; full flow shutter→low-confidence→recommendation). The first cold attempt timed out on emulator settle and passed on warm retry — environmental, not a regression.
- [x] Confirm `git status` shows **no** new image/binary files anywhere under `app/` or `docs/` (the spike commits text reports only), and **no** research API script is referenced by any Gradle app task, app source set, Android asset dir, or instrumentation test. → verified: no binaries; evidence dir is text-only; no app/gradle reference to the evidence dir.
...
- [x] **R5 — Accidental image commit / APK bloat.** *Mitigation:* eyeball samples in an off-repo scratch dir; evidence-folder `.gitignore`; Phase 4 `git status` asserts zero new binaries; nothing added to `app/src/main/assets` or `androidTest/assets`. → `.gitignore` in place; `git status` shows zero new binaries; evidence dir text-only.
- [x] **R6 — A query script smuggles a networking path into the app.** *Mitigation:* research tooling under `scripts/`/evidence dir, never invoked from a Gradle app task, never on device; `verifyNoNetworking` stays GREEN; Phase 4 asserts no app-source reference to it. → no script committed at all; `verifyNoNetworking` GREEN; no app/gradle reference to the evidence dir.
- [x] **R7 — Spike inflates into model work.** *Mitigation:* non-goals forbid any training/conversion/bundle/`ACTIVE_MODEL_ROOT` change; the only code diff allowed is the camera screen/VM + tests. → upheld; the only code diff is `CameraScreen.kt` + camera tests.
...
- [x] The shutter-on-return bug is fixed: after a capture, navigating back to the camera screen re-enables the shutter, proven by a failing-then-passing instrumented test plus a `CameraViewModelTest` unit case; existing named camera tests stay green. → `CameraShutterOnReturnTest` GREEN on GMD; `CameraViewModelTest.resetFromTerminalSuccessReturnsToIdle` GREEN; existing camera tests 6/6 GREEN.
- [x] No KB edits, no model/bundle/`ACTIVE_MODEL_ROOT` change, no new fixtures, no committed image data (verified by `git status`), and the `PlantIdentifier`/`IdentificationResult` seam is unchanged. → verified.
- [x] `ktlintCheck`, `testDebugUnitTest`, `verifyNoNetworking`, `scripts/check-stub-isolation.sh`, the instrumented camera tests on `pixel6Api34`, and `scripts/integration-flow.ps1` cold/warm/buildonly are GREEN — or carry a documented, accepted blocker in `docs/sprints/results/PLANTPOTTING-0008.md`. → all GREEN (cold-attempt emulator-settle flake documented; passed on warm retry).
- [x] `docs/sprints/results/PLANTPOTTING-0008.md` records the verdict, per-species count summary, the chosen hand-off, the UX-fix status, and all gate results; the ledger entry is flipped only after every gate is green. → results doc written; ledger flipped after gates green.
