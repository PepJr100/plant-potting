...
- [ ] **Push the `v0.7.0` git tag** (after gates pass) so `release.yml` publishes the public GitHub Release with the APK — a `versionName` bump alone never publishes; the workflow fires only on a pushed `v*` tag. *(DEFERRED by principal decision 2026-06-08: push the tag on `main` **after** execution PR #41 merges, so the public release points at reviewed/merged code. All gates already green; the version is bumped and the APK delivered — this is purely the outward-facing publish, gated on merge.)*
- [ ] **Confirm the GitHub Release workflow actually published** the APK (check the run + release asset), or record the exact CI failure. *(Follows the deferred tag push above — confirm after #41 merges + tag pushed.)*
