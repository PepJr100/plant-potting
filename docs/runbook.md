...
| Low-confidence picker always shows on emulator | Expected: AOSP's synthetic camera scene rarely clears the threshold. The active House Plant Species model (PLANTPOTTING-0007) maps 26 of its 47 classes to KB care cards (widened from 10 by PLANTPOTTING-0009); use a real device photo of an in-vocab species (snake plant, ZZ, monstera, jade, …) to hit the high-confidence path. |
