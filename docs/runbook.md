...
| Low-confidence picker always shows on emulator | Expected: AOSP's synthetic camera scene rarely clears the per-class threshold, and only ~2 KB species are in-vocab. Use a real device photo of an in-vocab species to hit the high-confidence path. |
