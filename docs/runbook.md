...
| Low-confidence picker always shows on emulator | Expected: AOSP's synthetic camera scene rarely clears the threshold. The active House Plant Species model (PLANTPOTTING-0007) covers 10 of the 16 KB species; use a real device photo of an in-vocab species (snake plant, ZZ, monstera, jade, …) to hit the high-confidence path. |
