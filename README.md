...
- **One model has one set of blind spots**, so:
  ...
  - *Execution* is single-model (one implementer, so the diff stays coherent) under a strict brief: failing test first, integration tests mandatory, `- [ ]` → `- [x]` checkboxes flipped in the plan file as each task lands. And 
